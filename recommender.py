...
class recommender:
    def __init__(self):
        ...
        self.__template = ("This is leaf has this disease {disease_name}."
                           "These are the conditions had by the leaf:"
                           "Temperature: {temperature} celsius."
                           "Humidity: {humidity}."
                           "consider above details, and recommend the following:"
                            "What are the best practices to cure this disease?"
                            "What are the best practices to prevent this disease?"
                            "how to control spread of this disease?"
                           )
        
        self.__prompt_template = PromptTemplate(template=self.__template, input_variables=["disease_name", "temperature", "humidity"])
    ...
    def getRecommendations(self, disease_name: str, temperature: float, humidity: float) -> list[str]:
        chain = self.__prompt_template | self.__model | self.__output_parser
        recomndations = chain.invoke({"disease_name": disease_name, "temperature": temperature, "humidity": humidity})
